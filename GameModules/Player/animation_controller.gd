...
func _physics_process(delta):
	...
	on_floor_blend = lerp(on_floor_blend, on_floor_blend_target, 1 * delta)
...
func _on_set_movement_state(_movement_state : MovementState):
	...
	tween = create_tween()
	tween.tween_property(animation_tree, "parameters/" + current_stance_name + "_movement_blend/blend_position", _movement_state.id, 0.1)
	tween.parallel().tween_property(animation_tree, "parameters/movement_anim_speed/scale", _movement_state.animation_speed, 0.2)
